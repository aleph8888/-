...
def list_generation():  #列表使用中括号，生成初始解空间
    ...
    for i in range(20):
        ...
        for j in range(2):
            ...
            if j==0:
                para_temp=random.randint(0,24)
            else:
                para_temp=random.randint(0,4)
            # elif j==2:
        ...
        temp=course_range(int(i/2),i,para[0],para[1])
        l.append(temp)
    return l
...
def teacher_course_task( l ): # 传入一个参数l  返回一个矩阵 记录每个老师每天上课的节数 10 x 5 的矩阵
    course_task=np.zeros((10,5),int)
    for i in range(20):
        teacher=l[i].teacher
        date=int(l[i].time/5) # 课程安排在周几
        course_task[teacher][date]+=1
...
def judge_conflict():
    ...
    # print(score[10])
    for ans in range(30):
        course_task = teacher_course_task(g[ans])
        for i in range(10):
            for j in range(5):
                if (course_task[i][j]>1):
                    score[ans] -=1
        for i in g[ans]:
            for j in g[ans]:
...
def main():
    for i in range(60):
        ...
        cross_action()
    for i in range(20):
        print('第{}条排课:\n'.format(i+1),g[0][i].teacher + 1, g[0][i].course + 1, g[0][i].time + 1, g[0][i].room + 1)
main()